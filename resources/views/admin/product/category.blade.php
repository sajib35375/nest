                        ...
                        <table id="table_id" class="table table-stripped table-bordered">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td><img style="width: 100px;height: 100px;" src="{{ URL::to('') }}/frontend/assets/imgs/icon/{{ $cat->icon }}"</td>
                                    <td>
                                        @if ($cat->status==true)
                                        <a class="btn btn-warning btn-sm" href="{{ route('product.category.inactive',$cat->id) }}">inactive</a>
                                        @else
                                        <a class="btn btn-sm btn-light" href="{{ route('product.category.active',$cat->id) }}">active</a>
                                        @endif
                                        <a id="edit_cat" edit_id={{ $cat->id }} class="btn btn-sm btn-info" href="{{ route('product.category.edit',$cat->id) }}">Edit</a>
                                        <a id="delete" class="btn btn-sm btn-danger" href="{{ route('product.category.delete',$cat->id) }}">Delete</a>
                                    </td>
                                </tr>
                                @endforeach
                            </tbody>
